...
function render(){
 ...
 const card=i=>'<article class="media-card"><button type="button" data-video-select="'+e(i.id)+'" aria-label="Watch '+e(i.title)+'"><div class="video-image"><img data-video="'+e(i.id)+'" src="https://i.ytimg.com/vi/'+e(i.id)+'/maxresdefault.jpg" alt="" loading="lazy">'+(i.duration?'<span class="video-duration">'+e(i.duration)+'</span>':'')+'<span class="video-play-cue" aria-hidden="true">▶</span></div><div class="media-card-copy"><span>'+e(i.series)+'</span><h3>'+e(i.title)+'</h3><p>'+e(date(i.publishedAt,true))+'</p></div></button></article>';
 const shelves=state.series==='all'&&!state.query.trim();
 $('#media-grid').classList.toggle('has-shelves',shelves);
 $('#media-grid').innerHTML=!items.length?'<div class="media-empty"><strong>No videos match.</strong><p>Try another series or search term.</p></div>':shelves?available.slice(1).filter(series=>items.some(i=>i.series===series)).map((series,index)=>'<section class="video-shelf" aria-labelledby="shelf-title-'+index+'"><div class="shelf-heading"><div><h2 id="shelf-title-'+index+'">'+e(series)+'</h2><p>'+e(descriptions[series])+'</p></div><div class="shelf-actions"><button type="button" data-shelf-step="-1" aria-label="Previous '+e(series)+' videos" aria-controls="shelf-'+index+'">←</button><button type="button" data-shelf-step="1" aria-label="Next '+e(series)+' videos" aria-controls="shelf-'+index+'">→</button></div></div><div class="shelf-track" id="shelf-'+index+'" tabindex="0" role="region" aria-label="'+e(series)+' videos">'+items.filter(i=>i.series===series).map(card).join('')+'</div></section>').join(''):items.map(card).join('');
...
$('#media-grid').addEventListener('click',event=>{const button=event.target.closest('[data-shelf-step]');if(!button)return;const track=document.getElementById(button.getAttribute('aria-controls'));track.scrollBy({left:Number(button.dataset.shelfStep)*track.clientWidth*.85,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});});
